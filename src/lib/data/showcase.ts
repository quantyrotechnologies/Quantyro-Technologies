...
export const DEFAULT_SHOWCASE_ITEMS: ShowcaseItem[] = [
  {
    ...
    metrics: [
      ...
      { label: 'Edge TTFB Latency', value: '< 25ms' },
  ...
  },
  {
    ...
    metrics: [
      ...
      { label: 'Organic Traffic Surge', value: '+4.8x' },
      { label: 'Indexing Speed', value: 'Instant' },
    ],
    accentColor: '#10B981',
    deviceType: 'laptop',
    screenType: 'seo',
  ...
  },
  {
    ...
    metrics: [
      ...
      { label: 'Manual Time Saved', value: '72%' },
    ],
    accentColor: '#0EBCD4',
    deviceType: 'mobile',
    screenType: 'ai',
  ...
  },
  {
    ...
    metrics: [
      { label: 'Compliance Level', value: 'HIPAA & SOC2' },
      { label: 'Platform Retention', value: '+4.2x' },
  ...
  try {
    ...
    return data.map((row) => ({
      ...
      metrics: Array.isArray(row.metrics)
        ? row.metrics
        : [
            { label: row.metric_1_label || 'Metric 1', value: row.metric_1_value || '100%' },
            { label: row.metric_2_label || 'Metric 2', value: row.metric_2_value || '99.9%' },
            { label: row.metric_3_label || 'Metric 3', value: row.metric_3_value || 'Sub-30ms' },
          ],
