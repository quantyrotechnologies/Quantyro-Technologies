...
export async function generateStaticParams() {
  ...
  return params.map((p) => ({ slug: p.industrySlug, capability: p.slug }));
}
...
export async function generateMetadata({
  ...
  const page = await findIndustrySolutionPage(slug, capability);
  if (!page) return {};
  return {
    title: page.seoTitle,
    description: page.seoDescription,
    alternates: { canonical: `/industries/${slug}/solutions/${capability}` },
    openGraph: { title: page.seoTitle, description: page.seoDescription, url: `/industries/${slug}/solutions/${capability}`, type: 'website', images: [DEFAULT_OG_IMAGE] },
...
export default async function IndustrySolutionDetailPage({
  ...
  const otherCapabilities = (await industrySolutionPagesForIndustry(slug)).filter((p) => p.slug !== capability);
      ...
      <section className="relative px-[6vw] pt-[150px] pb-[60px] z-10">
        <Breadcrumbs
          items={[
            ...
            { label: page.title, href: `/industries/${industry.slug}/solutions/${page.slug}` },
          ]}
        />
        ...
        <div className="mt-[20px] grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-[36px] lg:gap-[48px] items-center">
          <div>
            <div className="inline-flex items-center gap-[6px] px-[10px] py-[3px] rounded-full bg-[rgba(23,104,214,0.08)] border border-[rgba(23,104,214,0.18)] text-[var(--accent)] text-[11px] font-mono font-semibold uppercase mb-[14px]">
              ...
              <span>{industry.title} · Solution Deep Dive</span>
            </div>
          ...
          <div>
            ...
            <div className="mb-[56px]">
              ...
              <h3 className="text-[24px] md:text-[28px] font-bold font-[var(--font-display)] text-[var(--ink)] mb-[24px] max-w-[36ch]">
                Why This Matters for {industry.title}
              </h3>
            ...
            {otherCapabilities.length > 0 && (
              <div>
                ...
                >
                  ← Back to {industry.title} overview
                </Link>
              </div>
            )}
          </div>
